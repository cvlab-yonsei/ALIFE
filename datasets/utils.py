...
def filter_images(dataset, labels, labels_old=None, overlap=True):
    ...
    if comm.is_main_process(): print(f"Filtering images...")
    ...
    for i in range(len(dataset)):
        ...
        if fil(cls):
            idxs.append(i)
        if i % 1000 == 0:
            if comm.is_main_process(): print(f"\t{i}/{len(dataset)} ...")
    return idxs
...
class Subset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.target_transform is not None:
            new_target = self.target_transform[target.long()] #NOTE: Support "Long[Long]" ONLY 
